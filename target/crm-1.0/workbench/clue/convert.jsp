	...
	$(function(){
		...
		$("#aname").keydown(function (event) {
			if(event.keyCode == 13){
				...
				//alert("查询相关的市场活动")
				$.ajax({
					url:"workbench/clue/getActivityListByAname2.do",
					data:{
		...
		$("#convertBtn").click(function () {
			...
			if($("#isCreateTransaction").prop("checked")){
				//alert("为客户创建交易")
				//采用表单提交的方式提交数据,给需要提交的input选项添加name属性
				$("#tranForm").submit();

			}else {
				//alert("不为客户创建交易")
				window.location.href = "workbench/clue/convertTran.do?clueId=${param.id}";
			}
...
<body>
	...
	<div id="create-transaction2" style="position: relative; left: 40px; top: 20px; width: 80%; background-color: #F7F7F7; display: none;" >
		...
		<form id="tranForm" action="workbench/clue/convertTran.do" method="post">
